...
def H_UniaxialAnisotropy(mUnit, uAxis, Ku2, Msat):
    ...
    return Heff
'''
# ### Demagnetization Field function
def H_DemagField(mUnit,Msat):
    global Nxx, Nyy, Nzz, mu0
    hdx=-Nxx*mu0*Msat
    hdy=-Nyy*mu0*Msat
    hdz=-Nzz*mu0*Msat
    mArray=mUnit
    m_shape=numerix.asarray(numerix.shape(mArray))
    H_demag=numerix.zeros((m_shape[0],m_shape[1]),'d') #demag vector initialization
    H_demag[0]=hdx*mArray[0,:]
    H_demag[1]=hdy*mArray[1,:]
    H_demag[2]=hdz*mArray[2,:]
    # unit is in Tesla
    return H_demag
'''
...
#gamFac =  (1.76086e11)/(2*numerix.pi)                    # in 1/(s.T)
alphaDamping = 0.01
...
HdemagBase=H_DemagField(mAllCell,Msat)
...
while t_i<=number_of_steps:
    print('*************************************************************')
    percentage=float(float(t_i)/(float(number_of_steps+1)-1.0))*100.0
